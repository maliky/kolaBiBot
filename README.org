...
The bot can run a strategy file whose rows target different exchange routes. TSV rows may carry platform-specific symbols directly; no canonical symbol mapping table is used. Each active route needs matching public and private DB feeder evidence before live execution.

* Strategy Conditions

Each pair can be gated by:
- an entry window, =tps_run=, in minutes from launch;
- a typed price interval, =prix=;
...
The active TSV grammar puts the type prefix on each non-empty value:
- =prix= uses =D=, =%=, or =A= before its two bounds, for example =D- +=, =D- -5=, or =%0.20 0.30=.
- =qty= uses =A= or =%=, for example =A3=.
- =tp= uses =D=, =%=, or =A=, for example =D20= or =%0.10=.
- =oDelta= uses =D= or =%=, for example =D6= or =%0.20=; signed =D= values are accepted but materialised as side-aware positive distances.
- =tDelta= uses =D= when present.
- =tUblk= uses =D= or =%= when present, for example =D5= or =%0.20=. Empty keeps the default first-unblock distance: the initial tail distance plus observed spread.
- After =tUblk= unblocks the tail, the first managed update protects entry by jumping to the profit side of entry by four ticks plus the observed spread; later updates use the normal trailing curve.

Only empty typed fields receive implicit defaults. A non-empty typed value without its prefix is rejected. The old compact =atype= grammar is still accepted for compatibility files, but new files should not use it.

The =prix= field is a two-bound interval, not a single price. Use semi-open operator style in TSV files when appropriate, for example =D- -5= or =D5 +=, so the parser can translate the condition into a bounded internal interval.
...
Start other route lanes by selecting the same exchange and market type that appears in the TSV:
...
scripts/kolabi-fresh-run --strategy orders/demo_cross_exchange_chain.tsv --environment demo --symbol PI_XBTUSD
#+end_src
