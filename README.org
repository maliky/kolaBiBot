...
The runtime deliberately separates concerns:
- pure typed state transitions live in the reducer and pair-cycle functions;
- Chronos/StrategyRuntime orchestrates event flow and command deduplication;
- Ogun executes exchange commands through an adapter boundary;
- public/private DB rows are the platform-grounded evidence used by the bot.
...
The final =symbol= column is optional. When omitted, the CLI =--symbol= is used. When present, the row uses the platform-specific instrument directly, for example =PI_XBTUSD= or =PI_ADAUSD=. A multi-instrument TSV run requires one shared market DB URL and public feeders for every active symbol.
...
PostgreSQL is now the preferred local backend. SQLite fallback remains useful for small offline or legacy checks.
...
#+begin_src bash
python -m kolabi.bargain.cli --exchange kraken --environment demo instruments --contains ADA
