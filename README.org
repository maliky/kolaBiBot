...
Useful examples:
- =orders/demo_ada.tsv= documents order-type examples in the Org table grammar.
- =orders/demo_cross_exchange_chain.tsv= demonstrates the typed multi-exchange Org table grammar.
...
|- demo_ada.tsv                 # Org table contract and order-type examples
|- demo_kraken_xbt.tsv          # Kraken XBT grammar matrix
`- demo_cross_exchange_chain.tsv # multi-exchange chain scenario
