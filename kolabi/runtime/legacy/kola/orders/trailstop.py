...
class TrailStop(OrderConditionned):
    ...
    def __init__(
        ...
        main_oc,
        brg: BargainLike,
        pegOffset_perc: float = 0.5,
        updatepause: Optional[float] = None,
        logLevel_: str = "INFO",
        logpause: Optional[int] = 60,
    ...
    def __repr__(self, tracing: bool = False, short: bool = True, cond: bool = False) -> str:
        ret = "----TrailStop"
    ...
    def amend_stop_price(self, orderRefID: str):
        """
        Mise à jour du prix pour la condition de déclenchement (ie la trace).
        nécessite l'ID de l'ordre à changer (orderRefID)
        """
        # Cette methode est le coeur de la trace: elle met a jour le prix observe, recalcule la stopTail, puis n'envoie un amend que si la stopTail a change.
