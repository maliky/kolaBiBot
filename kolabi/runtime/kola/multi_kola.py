...
class KolaMarketAuditor:
    ...
    def go(
        ...
        while i < essais and not self.stop:
            ...
            if hook:
                ...
                ocp: Union[OrderConditionned, HookOrder] = HookOrder(
                    hSrc=_hSrc,
                    hStatus=_hStatus,
                ...
                ocp.add_condition(hookCond)
                ocp.condition.set_excludeClIDs(self.hookedIDs)
            else:
            ...
            msg = f"### OrdrePrincipal défini ### {repr(ocp)}"
            ...
            if hook:
                ...
                self.hookedIDs |= set([ocp.condition.hookedSrcID])
                self.logger.info(f">>>> Updating hookedIDs={self.hookedIDs}")

        self.fin_des_essais(essais, close=False)
        # Sortie conservee pour compatibilite CLI; a remplacer plus tard par un resultat de runtime.
        sys.exit()

    def record_new_executions(self, fout_: str = "./Logs/executions.csv") -> None:
        ...
        self.logger.info("First_execution")
    ...
    def fin_des_essais(self, essais: int, close: bool = False) -> None:
        """
