...
def amend_prices(
    ...
    try:
        ...
        # Commentaire FR: le demo Kraken peut renvoyer un 502 Cloudflare sur
        # /editorder alors que la session reste utilisable. On degrade en
        # reponse d'erreur non fatale pour laisser la boucle de trace continuer.
        if "Kraken HTTP 502" in str(exc) or "Kraken HTTP 503" in str(exc):
            mlogger.warning(f"Transient amend failure for {orderid}: {exc}")
            return {
                "orderID": orderid,
                "clOrdID": orderid,
                "error": "Transient amend gateway failure",
            }
        raise
