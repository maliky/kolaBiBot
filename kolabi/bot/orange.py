...
from kolabi.shared.core.runtime_types import BrokerReply, OrderQty, Symbol, to_decimal
...
def reason_from_status_or_reason(status: str, reason: str | None) -> OrderReason:
    ...
    if normalized in {"partiallyfilled", "partial_fill", HeadState.LIVING.value}:
        return OrderReason.PARTIAL_FILL
    if normalized in {"filled", "full_fill", HeadState.CLOSED.value}:
