"""Build operator-facing terminated-pair reports from Kolabi runtime logs.
...
def parse_log_text(text: str) -> dict[PairKey, PairLifecycle]:
    ...
    prices, fees, and liquidity when the final report is built.  Terminal
    `closed--living` and `closed--closed` updates can carry `0.0000` stop
    placeholders after the exchange has already filled the tail; those
    placeholders are ignored for amendment-diff calculations.
    """
    ...
    for raw_line in text.splitlines():
        ...
        if event == "HEAD_SENT":
            ...
            _parse_amend_sent(lifecycle, body, _parse_log_utc(match.group("log_ts")))
    return lifecycles
...
def build_report_table(
    ...
    *,
    ...
    lifecycles = parse_log_file(log_path)
    ...
    if not log_only:
        ...
        fill_summaries = fetch_fill_summaries(db_url, _client_ids(lifecycles.values()))
    rows = build_report_rows(
        lifecycles,
        fill_summaries=fill_summaries,
        require_db=not log_only,
    )
    return render_org_table(rows, options=options)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        ...
        description=(
            "Generate an aligned Org report table for terminated Kolabi pairs "
            "from a runtime log and local DB fill rows."
        ),
        epilog=(
            "Source order: logs provide lifecycle timing and amendments; the "
            "local account DB provides exact fills, fees, and maker/taker "
            "roles. Use --log-only only when DB rows are unavailable. A.diff "
            "is a quote-price difference per base unit, for example USD/ADA."
        ),
