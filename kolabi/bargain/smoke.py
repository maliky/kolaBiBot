...
def build_adapter(symbol: str, environment: str) -> Any:
    """Build the Kraken adapter from environment credentials."""
...
def build_smoke_orders(
    ...
    """Build a compact sweep of standard Kraken Futures order kinds."""
    del symbol
...
def run_smoke(
    symbol: str, environment: str, sleep_seconds: float, log_level: str
) -> int:
    """Run the standard-order smoke test against Kraken Futures."""
    logging.basicConfig(
        level=getattr(logging, log_level.upper(), logging.INFO),
        format="%(asctime)s %(levelname)s %(message)s",
    )
    logger = logging.getLogger("kraken_smoke")
    adapter = build_adapter(symbol, environment)
    instrument = adapter.validate_symbol(symbol)
    quantity = extract_min_quantity(instrument)
    reference_price = extract_reference_price(adapter, symbol)
    logger.info(
        "smoke_start symbol=%s environment=%s reference_price=%.4f quantity=%.4f",
        symbol,
        environment,
        reference_price,
        quantity,
    )
    orders = build_smoke_orders(symbol, quantity, reference_price)
    for order in orders:
        logger.info(
